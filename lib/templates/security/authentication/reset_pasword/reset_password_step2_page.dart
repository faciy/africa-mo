...
import 'package:innovimmobilier/templates/security/authentication/reset_pasword/reset_password_step3_page.dart';
import 'package:innovimmobilier/utilities/constants/colors.dart';
import 'package:innovimmobilier/utilities/constants/dimensions.dart';

class ResetPasswordStep2Page extends StatefulWidget {
  ResetPasswordModel resetPasswordModel;
  ResetPasswordStep2Page({Key? key, required this.resetPasswordModel})
      : super(key: key);
...
class _ResetPasswordStep2PageState extends State<ResetPasswordStep2Page> {
  ...
  _goToStep3(BuildContext context,
      ResetPasswordController resetPasswordController) async {
    String code = _codeController.text.trim();

    if (code.isEmpty) {
      showCustomSnackBar(
        "Veuillez saisir votre code reçu par email svp!",
        type: 'error',
        context: context,
      );
    } else {
      ...
      resetPasswordController
          .resetPassword(resetPasswordModel, 2)
          .then((status) {
        if (status.isSuccess) {
          Get.to(
            () => ResetPasswordStep3Page(
              resetPasswordModel: resetPasswordModel,
            ),
